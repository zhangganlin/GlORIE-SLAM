import torch
import warnings
from src.utils.common import get_rays, raw2outputs_nerf_color


class Renderer(object):
    def __init__(self, cfg, slam, points_batch_size=500000, ray_batch_size=3000):
        ...
        self.sigmoid_coefficient = cfg['rendering']['sigmoid_coef']
        self.near_end = cfg['rendering']['near_end']

        self.use_dynamic_radius = cfg['pointcloud']['use_dynamic_radius']

        # self.H, self.W, self.fx, self.fy, self.cx, self.cy = \
        #     slam.H_o, slam.W_o, slam.fx_o, slam.fy_o, slam.cx_o, slam.cy_o
        self.H, self.W, self.fx, self.fy, self.cx, self.cy = \
    ...
    def render_batch_ray(self, npc, decoders, rays_d, rays_o, device, stage, gt_depth=None,
        ...
        near_end_surface = self.near_end_surface
        far_end_surface = self.far_end_surface
    ...
    def render_img(self, npc, decoders, c2w, device, stage, gt_depth=None,
        ...
        Returns:
            ...
            valid_ray_mask (tensor, H*W): indicate whether each unprojected ray is valid or not
            valid_ray_count (tensor, H*W): how many valid points along each unprojected ray
        """
        H = self.H
        W = self.W
