...
class MLP_geometry(nn.Module):
    ...
    def forward(self, p, npc, npc_geo_feats, pts_num=16, is_tracker=False, cloud_pos=None,
        ...
        valid_ray_mask = ~(
            torch.sum(has_neighbors.view(-1, pts_num), 1) < 3)
        valid_ray_counter = torch.sum(has_neighbors.view(-1, pts_num), 1)
        ...
        # (N,1)->(N,) for occupancy
        out = out.squeeze(-1)
        return out, valid_ray_mask, has_neighbors, valid_ray_counter
...
class MLP_color(nn.Module):
    ...
    def forward(self, p, npc, npc_col_feats, is_tracker=False, cloud_pos=None, 
                pts_views_d=None, dynamic_r_query=None):
...
class POINT(nn.Module):
    ...
    def __init__(self, cfg, c_dim=32,
        ...
        self.geo_decoder = MLP_geometry(cfg=cfg, c_dim=c_dim,
                                        ...
                                        pos_embedding_method=pos_embedding_method).eval()
    ...
    def forward(self, p, npc, stage, npc_geo_feats, npc_col_feats, pts_num=16, is_tracker=False, cloud_pos=None,
        ...
        match stage:
            case 'geometry':
                geo_occ, ray_mask, point_mask, ray_counter = self.geo_decoder(p, npc, npc_geo_feats,
                ...
                raw[..., -1] = geo_occ
                return raw, ray_mask, point_mask, ray_counter
            case 'color':
                geo_occ, ray_mask, point_mask, ray_counter = self.geo_decoder(p, npc, npc_geo_feats,
                                                                 pts_num=pts_num, is_tracker=is_tracker, cloud_pos=cloud_pos,
                                                                 dynamic_r_query=dynamic_r_query)
