...
class SLAM:
    ...
    def load_pretrained(self, cfg):
        ...
        self.droid_net.eval()
        print(f'INFO: load droid pretrained checkpiont from {droid_pretrained}!')

    def tracking(self, stream:BaseDataset, pipe):
        ...
        while(self.all_trigered < self.num_running_thread):
            pass
        self.tracker.run(stream)
        print('Tracking Done!')
        if self.only_tracking:
            self.terminate(stream=stream)
    
    def mapping(self, stream:BaseDataset, pipe):
        if self.only_tracking:
            self.all_trigered += 1
            return
    ...
    def backend(self):
        ...
        torch.cuda.empty_cache()
        self.ba.dense_ba(12)
        print("Backend Optimization Done!")
                    

    def terminate(self, stream:BaseDataset=None):
    ...
    def run(self, stream:BaseDataset):
        ...
        processes = [
            mp.Process(target=self.tracking, args=(stream, t_pipe)),
            mp.Process(target=self.mapping, args=(stream, m_pipe)),
        ]
